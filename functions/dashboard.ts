...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .retention-cell {
    ...
    width: 44px; height: 24px; line-height: 24px;
...
function render(data) {
  ...
  html += card('New Users', fmt(today.new_users), fmtDelta(today.new_users, yesterday.new_users), 'var(--pink)');
  html += '</div>';

  html += '<div class="charts">';
  html += chartCard('DAU Trend', 'full', 'dau-chart');
  html += chartCard('Messages / New Users', '', 'msg-chart');
  html += chartCard('Version Dist.', '', 'version-chart');
  html += chartCard('OS Dist.', '', 'os-chart');
  html += '</div>';

  if (data.retention?.cohorts?.length) {
    html += '<div class="charts"><div class="chart-card full">';
    html += '<div class="chart-title"><div class="dot" style="background:var(--purple)"></div>Retention</div>';
    html += '<table class="retention-table"><thead><tr><th>Date</th><th>New</th><th>D+1</th><th>D+3</th><th>D+7</th><th>D+14</th><th>D+30</th></tr></thead><tbody>';
    for (const c of data.retention.cohorts.slice(0, 15)) {
      ...
      for (const offset of [1,3,7,14,30]) {
        ...
        html += '<td>' + (val == null ? '-' : '<span class="retention-cell" style="background:' + retentionColor(val) + '">' + val + '%</span>') + '</td>';
      }
      html += '</tr>';
    }
  ...
  new Chart(document.getElementById('dau-chart'), {
    ...
    data: {
      ...
      datasets: [
        ...
        { label: 'Eff. DAU', data: trends.map(t => t.eff_dau), borderColor: '#34d399', backgroundColor: 'rgba(52,211,153,0.08)', fill: true, tension: 0.3, pointRadius: 2 },
      ]
    },
    options: chartOpts(),
  });
  ...
  new Chart(document.getElementById('msg-chart'), {
    ...
    data: {
      labels,
      datasets: [
        { label: 'Messages', data: trends.map(t => t.total_msgs), backgroundColor: 'rgba(96,165,250,0.6)', borderRadius: 3, barPercentage: 0.6 },
        { label: 'New Users', data: trends.map(t => t.new_users), backgroundColor: 'rgba(244,114,182,0.6)', borderRadius: 3, barPercentage: 0.6 },
      ]
    },
    options: chartOpts(),
  });
...
function chartOpts() {
  return {
    ...
    plugins: { legend: { labels: { color: '#8888a0', font: { size: 11 }, usePointStyle: true, pointStyle: 'circle' } } },
    scales: {
...
async function load() {
  try {
    const data = await fetchData(document.getElementById('range').value);
    render(data);
  } catch (e) {
